"""
Kaggle-optimized Python SDK & Pydantic Documentation Processing Pipeline
For GPU T4 x2 with nomic-ai/nomic-embed-code

This script processes Python SDK and Pydantic markdown documentation:
1. Chunks markdown files (heading-based splitting)
2. Generates embeddings
Collection name: python_sdk_and_pydantic
"""
...
# Paths
INPUT_DIR = Path("Docs/python_sdk_and_pydantic")
OUTPUT_CHUNKED_DIR = Path("output/python_sdk_and_pydantic/chunked")
OUTPUT_EMBEDDINGS_DIR = Path("output/python_sdk_and_pydantic/embeddings")
...
def chunk_markdown_file(content: str, source_file: str, subdir: str) -> List[Dict]:
    ...
    Args:
        ...
        subdir: Subdirectory (pydantic or python-sdk)
...
def chunk_documents() -> List[Dict]:
    ...
    for subdir in ["pydantic", "python-sdk"]:
...
def main():
    ...
    print(f"\n{'='*60}")
    print("PYTHON SDK & PYDANTIC EMBEDDING PIPELINE")
    print(f"{'='*60}")
    ...
    print(f"{'='*60}")
    print(f"Total markdown files processed: ~308")
