"""
Kaggle-optimized Inngest Documentation Processing Pipeline
For GPU T4 x2 with nomic-ai/nomic-embed-code

This script processes Inngest documentation from 3 subdirectories:
- inngest/ (12 files)
- inngest-js/ (13 files) 
- inngest-py/ (31 files)

Collection name: inngest_overall

UNIQUE ID STRATEGY:
- IDs include subdirectory to prevent conflicts across folders
- Format: inngest_overall:{subdir}:{filename}:chunk:{index}
- Example: inngest_overall:inngest:Overview.md:chunk:0
- Example: inngest_overall:inngest-js:Overview.md:chunk:0  ← Different ID!
- Example: inngest_overall:inngest-py:Overview.md:chunk:0  ← Different ID!
"""
...
# Paths
INPUT_DIR = Path("Docs/inngest")
OUTPUT_CHUNKED_DIR = Path("output/inngest_overall/chunked")
OUTPUT_EMBEDDINGS_DIR = Path("output/inngest_overall/embeddings")

# Subdirectories to process
SUBDIRS = ["inngest", "inngest-js", "inngest-py"]
...
def chunk_markdown_file(content: str, source_file: str, subdir: str) -> List[Dict]:
    ...
    Args:
        ...
        subdir: Subdirectory name (inngest, inngest-js, inngest-py)
...
def chunk_documents() -> List[Dict]:
    ...
    for subdir in SUBDIRS:
        ...
        output_subdir = OUTPUT_CHUNKED_DIR / subdir
        ...
        for md_file in md_files:
            try:
                ...
                chunks = chunk_markdown_file(content, md_file.name, subdir)
        ...
        print(f"\n  Subtotal for {subdir}/: {subdir_chunk_count} chunks")
...
def main():
    ...
    print(f"\n{'='*60}")
    print("INNGEST DOCUMENTATION EMBEDDING PIPELINE")
    print(f"{'='*60}")
    ...
    print(f"   Example: {COLLECTION_NAME}:inngest:Overview.md:chunk:0")
    print(f"   Example: {COLLECTION_NAME}:inngest-js:Overview.md:chunk:0")
    ...
    print(f"Total markdown files: ~56 (12 + 13 + 31)")
