"""
Kaggle-optimized Agent Kit GitHub Documentation Processing Pipeline
For GPU T4 x2 with nomic-ai/nomic-embed-code

This script processes Agent Kit documentation:
- agent-kit_github/ (19 markdown files)

Collection name: agent_kit

UNIQUE ID STRATEGY:
- Format: agent_kit:{filename}:chunk:{index}
- Example: agent_kit:Overview.md:chunk:0
"""
...
# Paths
INPUT_DIR = Path("Docs/agent-kit_github")
OUTPUT_CHUNKED_DIR = Path("output/agent_kit/chunked")
OUTPUT_EMBEDDINGS_DIR = Path("output/agent_kit/embeddings")
...
def chunk_markdown_file(content: str, source_file: str) -> List[Dict]:
    ...
    ID FORMAT: {COLLECTION_NAME}:{filename}:chunk:{index}
...
def chunk_documents() -> List[Dict]:
    """
    Process all markdown files from agent-kit_github directory
...
def main():
    ...
    print(f"\n{'='*60}")
    print("AGENT KIT GITHUB DOCUMENTATION EMBEDDING PIPELINE")
    print(f"{'='*60}")
    ...
    print(f"{'='*60}")
    print(f"Total markdown files processed: ~19")
